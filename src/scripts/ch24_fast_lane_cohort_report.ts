...
import { buildFastLaneCohortReport } from "../lib/quality/fastLaneCohortReport";
...
async function main() {
  ...
  const outputPath =
    ...
    "docs/reports/CH24_FAST_LANE_COHORT_REPORT.json";

  const report = await buildFastLaneCohortReport({ windowDays, limit });
