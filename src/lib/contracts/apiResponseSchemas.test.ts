...
test("attempt details contract accepts current shape", () => {
  const parsed = attemptDetailsResponseSchema.parse({
    ...
    results: {
      ...
      feedback: {},
      causal: {
        taxonomyVersion: "causal-taxonomy-v1",
        modelVersion: "causal-inference-v1",
        topLabel: "rule_confusion",
        topProbability: 0.62,
        entropy: 0.51,
        topMargin: 0.24,
        distribution: [{ label: "rule_confusion", p: 0.62 }],
        confidenceInterval: { lower: 0.52, upper: 0.72 },
        counterfactual: { suggestedRemediation: "Run grammar contrast." },
        createdAt: new Date(),
      },
      gseEvidence: [],
      evidenceMatrix: [],
