...
async function main() {
  ...
  const outputPath =
    parseStringFlag(argv, "--output") || "docs/reports/CH38_LISTENING_TRANSFER_RETENTION_REPORT.json";

  const report = await buildListeningTransferRetentionReport({
    windowDays,
    limit,
  });
    ...
    JSON.stringify(
      {
        ...
        summary: {
          ...
          listeningAttempts: report.listeningAttempts,
          scoredListeningAttempts: report.scoredListeningAttempts,
          avgTaskScore: report.avgTaskScore,
          passRate: report.passRate,
          transferEvaluable: report.transfer.evaluableCount,
          transferPassRate: report.transfer.passRate,
          retention7dCount: report.retention.window7dCount,
          retention30dCount: report.retention.window30dCount,
        },
      },
      null,
      2,
