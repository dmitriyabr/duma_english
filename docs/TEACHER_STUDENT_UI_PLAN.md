...
### Ученик
- **Вход:** `/login` — код класса + имя + возрастная группа. Если ученика с таким именем в классе нет — создаётся при первом входе.
...
### Учитель
1. **Создавать классы** — уже есть; нужно встроить в единый дашборд.
2. **Создавать учеников** — явное добавление ученика в класс (имя), без обязательного входа ученика.
3. **Генерировать коды** — уже есть (один код на класс); показывать код в интерфейсе класса, при необходимости генерировать новый.
4. **Видеть в классе учеников** — список учеников класса с базовой инфой (имя, дата создания, последняя активность, стадия).
...
### Ученик
- Вход по коду учителя (коду класса) — **как сейчас**; дальше весь поток без изменений (home → task → record → results, progress).
...
- API учителя: `GET/POST /api/teacher/classes`, `GET /api/teacher/classes/[classId]`, `POST /api/teacher/classes/[classId]/students`, `POST /api/teacher/classes/[classId]/codes`, `GET /api/teacher/students/[studentId]`.
...
- «Код учителя» = код класса; ученик входит по коду класса и имени — без изменений.
