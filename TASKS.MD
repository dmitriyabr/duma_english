...
15. **CH-41 (blueprint invariants):** Regression test pack `src/lib/invariants/blueprintInvariants.test.ts` encodes causal, transfer, retention, retry, policy/reward invariants; `npm run test:invariants`; CI workflow `Blueprint Invariants` runs on PR/push.

16. **CH-40 (model/prompt registry):** All judge/generator/causal/policy model and prompt versions are centralised in `getModelPromptRegistry()` (`src/lib/registry/modelPromptRegistry.ts`); GET `/api/quality/model-prompt-registry`, `npm run model-prompt-registry:report` → `docs/reports/CH40_MODEL_PROMPT_REGISTRY.json`; immutable release tag via `REGISTRY_RELEASE_TAG` or date-based `release-YYYY-MM-DD`.

17. **CH-38 / CH-39 (listening transfer-retention, cross-modality placement):**  
